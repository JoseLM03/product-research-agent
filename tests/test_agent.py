...
def run(model, search=None, limit=10):
    ...
    result=asyncio.run(research(ResearchInput(idea='compact coffee grinder'), model, ResearchTools(search or SearchFixture()), limit, emit))
...
def test_native_tool_loop_observes_results_then_submits():
    ...
    assert [e[0] for e in events if e[1]=='completed']==['search_competitors','calculate_margin','submit_report']


def test_invalid_citation_can_be_repaired_but_is_disclosed():
    model=ModelFixture([call('search_web',{'query':'grinder'}),call('submit_report',draft('S9')),call('submit_report',draft())])
    (report,status),events=run(model)
...
def test_provider_failure_is_observed_and_disclosed():
    class Intermittent:
        ...
        async def search(self,query):
            self.count+=1
            if self.count==1: raise ProviderError('Search unavailable.')
            return await SearchFixture().search(query)
    (report,status),_=run(ModelFixture([call('search_web',{'query':'grinder'}),call('search_competitors',{'query':'grinder'}),call('submit_report',draft())]),Intermittent())
    assert status=='partial'
