...
def test_api_to_worker_to_persisted_report(client, app):
    ...
    asyncio.run(
        run_job(job_id, app.state.sessions, app.state.settings, ModelFixture(), SearchFixture())
    )
...
def test_csrf_input_limits_and_security_headers(client):
    assert (
        client.post(
            "/api/research", json={"idea": "grinder"}, headers={"origin": "https://evil.example"}
        ).status_code
        == 403
    )
...
def test_concurrent_job_prevention_and_daily_budget(client, app):
    ...
    assert (
        client.request("DELETE", "/api/research/" + first.json()["id"], json={}).status_code == 200
    )
