...
async def research(request: ResearchInput, model, tools, max_calls, emit):
    ...
    for _ in range(8):
        ...
        if not calls:
            messages.append({'role': 'user', 'content': 'Use the available tools. Finish with submit_report; free text is not a report.'})
            continue
        for call in calls:
            used += 1
            if used > max_calls:
                raise AgentError('Research stopped at its tool-call limit. No validated report was produced.')
            ...
            try:
                ...
                if name == 'submit_report':
                    ...
                    await emit(name, 'completed', 'Report structure and source references validated.')
                    return report, ('partial' if tools.failures or len(tools.sources) < 2 else 'completed')
                result = await tools.execute(name, args)
                await emit(name, 'completed', f'{len(result["sources"])} sources returned.' if 'sources' in result else 'Tool finished.')
            except (ValidationError, ValueError, ProviderError) as exc:
                detail = str(exc) if isinstance(exc, ProviderError) else 'Invalid tool arguments or unsupported citation. Use the tool schema and exact source quotes.'
