...
async def bounded_json(client, method, url, *, limit=500_000, retry=False, **kwargs):
    ...
    for attempt in range(attempts):
        try:
            ...
            if retry and isinstance(exc, (httpx.ConnectError, httpx.ConnectTimeout)) and attempt + 1 < attempts:
                await asyncio.sleep(0.5)
                continue
            raise ProviderError('Provider unavailable, timed out, or returned an invalid response.') from None
...
class Ollama:
    ...
    async def chat(self, messages, tools):
        body = await bounded_json(self.client, 'POST', self.settings.ollama_url.rstrip('/') + '/api/chat',
            json={'model': self.settings.ollama_model, 'messages': messages, 'tools': tools,
                  'stream': False, 'think': False, 'options': {'temperature': 0, 'num_predict': 2500, 'num_ctx': 16384}},
            timeout=90)
        ...
        return {'role': 'assistant', 'content': str(message.get('content', ''))[:16000], 'tool_calls': calls}
...
class BraveSearch:
    ...
    async def search(self, query):
        ...
        body = await bounded_json(self.client, 'GET', 'https://api.search.brave.com/res/v1/web/search', retry=True,
            params={'q': query, 'count': 5, 'safesearch': 'strict', 'text_decorations': 'false'},
            headers={'X-Subscription-Token': self.key, 'Accept': 'application/json'}, timeout=15)
        try:
