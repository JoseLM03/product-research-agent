...
from .db import Job, ToolEvent, Admission
...
async def run_job(job_id, sessions, settings, model, search):
    ...
    try:
        async with asyncio.timeout(settings.job_timeout_seconds):
            report, status = await research(request, model, ResearchTools(search, request.costs), settings.max_tool_calls, emit)
        ...
        with sessions.begin() as db:
            db.execute(update(Job).where(Job.id == job_id, Job.status == 'running').values(
                status=status, report=report, error=error, finished_at=time.time()))
    ...
    while True:
        ...
        try:
            with sessions.begin() as db:
                ...
                db.execute(update(Job).where(Job.status == 'running', Job.started_at < now - settings.job_timeout_seconds - 60)
                    .values(status='failed', error='Worker interrupted. Start a new research task.', finished_at=now))
                if now - last_cleanup > 3600:
                    db.execute(delete(Job).where(Job.created_at < now - settings.retention_days * 86400, Job.status.not_in(['queued', 'running'])))
                ...
                claimed = None
                if candidate:
                    claimed = db.scalar(update(Job).where(Job.id == candidate, Job.status == 'queued')
                        .values(status='running', started_at=now).returning(Job.id))
            if claimed:
